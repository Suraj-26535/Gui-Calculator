...
def press(num):
    ...
    equation.set(expression)# update the expre. using set method.
def equalpress():# for evaluating final expression.
    try:
    ...
    except:# used to handle the expression.
        equation.set(" error ")
        expression = "" 
...
if __name__ == "__main__":  
    gui = Tk()    # for GUI window
    gui.configure(background="white") #for background color of gui window.
    gui.title("Simple Calculator")   # set the title for window
    gui.geometry("270x150") #  configuration of GUI window
 
    # StringVar() is the variable class.we create an instance of this class.
    equation = StringVar()
 
    # text entry box for showing the expression .
